...
int libraryFine(int d1, int m1, int y1, int d2, int m2, int y2) {
    ...
    // If returned after due year
    if (y1 > y2) return 10000;

    // If returned after due month (same year)
    if (y1 == y2 && m1 > m2) return 500 * (m1 - m2);

    // If returned after due day (same month & year)
    if (y1 == y2 && m1 == m2 && d1 > d2) return 15 * (d1 - d2);

    return 0; // default case (though all paths covered)
}
